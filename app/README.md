...
- **メイン保存先**: FastAPI backend (`/api/v1/containers`)
- **キャッシュ**: AsyncStorage
- **キー**: `@trash_bins_v1`
- **形式**: JSON 配列
- **API接続不可時**: キャッシュ、またはダミーデータ1件（岡山駅東口 ベンチ横）を表示
...
バックエンドを起動してから、別ターミナルでアプリを起動します。
...
必要に応じて `app/.env` に API URL を指定できます。実機の Expo Go で確認する場合は、PC の LAN IP を指定してください。
